...
public:
    ...
    size_t read(double *buf, int size);
};
...
void WaveFileReader::readHeader(){
    ...
    //printf("wave format details: %d,%d,%d,%d",wav_format.num_channels,wav_format.sample_rate,wav_format.block_align,wav_format.sig_bps);
}

size_t WaveFileReader::read(double *circBuf, int size){
    unsigned channel;
	...
	temp = (double)slice_value/32768.0;
	circBuf[i] = temp;
    }
    free(slice_buf);
    return size;
