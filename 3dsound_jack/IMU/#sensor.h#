...
{
    int result = i2c_smbus_read_i2c_block_data(file1, command, size, data);
    if (result != size)
    {
...
void selectDevice(int file1, int addr)
{
        char device[3];
        if (addr == 1)
                device == "L3G";
...
{
 	uint8_t block[6];
        selectDevice(file1,ACC_ADDRESS);
...
{
	uint8_t block[6];
        selectDevice(file1,MAG_ADDRESS);
...
void readGYR(int *g)
{
	uint8_t block[6];
        ...
        selectDevice(file1,GYR_ADDRESS);
...
{
    selectDevice(file1,ACC_ADDRESS);
  int result = i2c_smbus_write_byte_data(file1, reg, value);
    if (result == -1)
    {
